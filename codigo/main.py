...
# ---> Configurações iniciais para o ambiente e a IA <--- #
...
# ---> Criação interface com Streamlit <--- #
st.title("Painel de Controle da Missão")
st.subheader("Telemetria em tempo real e status do sistema")
# 1. as sidebar menu
with st.sidebar:
    selected = option_menu("Projeto Aurora", ["Dashboard", 'Verificação', "Simulação", "Energia"], 
        icons=['bi bi-speedometer', 'bi bi-activity', "bi bi-ui-checks", "bi bi-battery-charging"], menu_icon="bi bi-rocket", default_index=1)
    selected
...
if testes_sucessos == 3:
    ...
    print(
        f"{time.localtime().tm_hour}:{time.localtime().tm_min}:{time.localtime().tm_sec} - A equipe de engenharia está investigando as falhas.\n"
    )
