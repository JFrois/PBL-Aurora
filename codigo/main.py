...
# Versão atualizada com Fase 5: NCAS (Núcleo Cognitivo da Aurora Siger)
# ==============================================================================
...
from codigo_fonte import executar_fase5  # ← NOVO: Fase 5 (NCAS)
...
def rodar_simulacao_integrada():
    ...
    # ------------------------------------------------------------------
    # FASE 5 — NCAS: registra/consulta dados operacionais, aplica a regra
    # lógica sobre os alertas e (se houver client) já usa a IA para
    # analisar os alertas críticos. Repassa o client para reaproveitar
    # a mesma sessão em vez de abrir uma nova.
    # ------------------------------------------------------------------
    res_f5 = executar_fase5(client)  # ← NOVO BLOCO

    # ------------------------------------------------------------------
    # Consolidação do Data Lake (todas as 5 fases)
    # ------------------------------------------------------------------
    contexto_operacional = {
        "Fase_1_Lancamento": res_f1,
        "Fase_2_Pouso": res_f2,
        "Fase_3_Colonia": res_f3,
        "Fase_4_SIGIC_Grafos": res_f4,
        "Fase_5_NCAS": res_f5,  # ← NOVO BLOCO
    }
    ...
    # ------------------------------------------------------------------
    # IA 1: Analista Técnico (correlação entre as 5 fases)
    # ------------------------------------------------------------------
    print("\n" + "=" * 85)
    print("PROCESSAMENTO DE DADOS E AUDITORIA (IA)".center(85))
    print("=" * 85)
    print(">> Extraindo correlações entre as 5 fases...")

    prompt_contexto = (
        f"Você é um Engenheiro de Sistemas Aeroespaciais sênior. "
        f"Analise os logs completos da missão (5 fases):\n{dados_json}\n\n"
        ...
        f"Em seguida, avalie os alertas críticos identificados pelo Núcleo Cognitivo "
        f"(Fase 5 - NCAS) e se as respostas geradas pelo assistente são adequadas. "
    ...
    # ------------------------------------------------------------------
    # IA 2: Diretor de Voo (boletim executivo com as 5 fases)
    # ------------------------------------------------------------------
    print(">> Sintetizando Boletim Executivo (5 fases)...\n")
    ...
    prompt_executivo = (
        ...
        f"FASE 5: NCAS — NÚCLEO COGNITIVO\n"
        f"-> [Análise dos alertas críticos e da resposta do assistente inteligente]\n\n"
